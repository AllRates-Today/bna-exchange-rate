# National Bank of Angola Exchange Rates API — bna-exchange-rate
...
The latest National Bank of Angola table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/bna?source=USD&target=AOA"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/bna').then((x) => x.json());
...
Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/bna-exchange-rate`](https://www.npmjs.com/package/@allratestoday/bna-exchange-rate) — same code, same versions.
...
The National Bank of Angola publishes daily reference kwanza rates — mid, buying and selling — for some 60 currencies. As the central bank of one of Africa's largest oil exporters, its reference rate anchors import pricing and repatriation accounting, with history reaching the 1999 kwanza reform.

- Publisher's own page: [Taxas de câmbio](https://www.bna.ao/#/pt/mercados) · [www.bna.ao](https://www.bna.ao)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [National Bank of Angola rates page](https://allratestoday.com/central-bank-rates-api/bna/)
...
- National Bank of Angola quotes **AOA per 1 unit of foreign currency** (e.g. `base: "USD", quote: "AOA"` means AOA per one US dollar).
...
Loading the official National Bank of Angola rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/bna/latest?format=xml&api_key=art_live_..."
```
...
- [Package docs on the site](https://allratestoday.com/docs/sdk/bna-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
